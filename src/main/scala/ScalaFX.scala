...
import scalafx.scene.control.{CheckBox, Button, Label}
...
import scalafx.scene.input.MouseEvent
import scalafx.scene.layout.{VBox, HBox, BorderPane, Pane}
import scalafx.scene.{Scene, Group, Node}

object ScalaFX extends JFXApp {
  ...
  /*
      scene = new Scene {
        root = new BorderPane {
          ...
          bottom = new Button {
            text = "[bottom] Click me to close the dialog"
            onAction = handle {
              outer.close()
            }
          }
